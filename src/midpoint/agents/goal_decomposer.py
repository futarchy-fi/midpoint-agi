    ...
    async def determine_next_step(self, context: TaskContext, setup_logging=False, debug=False, quiet=False) -> SubgoalPlan:
        ...
        try:
            ...
            if all(key in output_data for key in ["next_step", "validation_criteria", "reasoning"]):
                ...
                # Generate goal ID for this subgoal
                goal_id = self.generate_goal_id(parent_goal_filename.replace('.json', ''))
                ...
                final_output = SubgoalPlan(
                    ...
                    metadata={
                        ...
                        "parent_goal_file": parent_goal_filename,
                        "goal_id": goal_id
                    }
                )
            else:
...
"""
    ...
    def create_top_goal_file(self, context: TaskContext, logs_dir="logs") -> str:
        ...
        # Write the goal file
        output_file = logs_path / f"{goal_id}.json"
        ...
        for file_path in logs_path.glob("*.json"):
            try:
                with open(file_path, 'r') as f:
                    ...
                    subgoal_files.append((str(file_path), timestamp, next_step, parent_goal, goal_id))
            except:
                continue
