...
"""
    ...
    def generate_goal_id(self, parent_id=None, logs_dir="logs", is_task=False):
        """
        Generate a goal ID in format G1 or S1.
        
        Uses 'G' prefix for top-level goals and 'S' prefix for subgoals.
        The is_task parameter is deprecated and no longer affects naming.
        ...
        # Determine prefix based on parent_id (is_task is deprecated, not used)
        if parent_id:
            # Subgoal: use 'S' prefix
            prefix = "S"
        else:
            # Top-level goal: use 'G' prefix
            prefix = "G"
        
        # Find next available number for the prefix
        max_num = 0
        for file_path in logs_path.glob(f"{prefix}*.json"):
        ...
        # Next number is one more than the maximum found
        next_num = max_num + 1
